...
export default function Button({
    ...
    const _className = React.useMemo(() => {
        ...
        switch (color) {
            case 'light-green':
                cl += ' ' + 'bg-light-green hover:bg-light-green/90 active:bg-light-green/70';
                break;

            default:
                cl += ' ' + 'bg-dark-green hover:bg-dark-green/90 active:bg-dark-green/70';
                break;
        }
        if (rounded) cl += ' ' + 'rounded-[50px]';
        if (className) cl += ' ' + className;
        if (isLoading) cl += ' !cursor-wait !bg-gray-400';

        return cl;
    }, [className, isLoading]);

    return (
        <button disabled={disabled || isLoading} className={_className} type={type} {...props}>
            {isLoading ? (
                <div className="grid place-items-center overflow-x-scroll rounded-lg lg:overflow-visible">
                    <svg
                        className="text-gray-300 animate-spin size-5"
                        viewBox="0 0 64 64"
                        fill="none"
