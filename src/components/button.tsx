...
const Button = ({ children, className, isLoading, ...props }: ButtonProps) => {
    const _className = React.useMemo(() => {
        let cl = `bg-dark-green text-light py-2 px-3 rounded-md font-semibold text-md`;

        if (className) cl += ' ' + className;

        if (isLoading) cl += ' !bg-gray-500 cursor-wait';

        return cl;
    }, [className, isLoading]);

    return (
        <button className={_className} disabled={isLoading} {...props}>
            {isLoading ? (
                <svg
                    aria-hidden="true"
                    role="status"
