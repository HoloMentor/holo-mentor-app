    ...
    return (
        <div className="w-full h-auto max-w-md p-4 bg-white shadow-custom rounded-xl drop-shadow-sm">
            <div className="flex items-center justify-between">
                <span className="pl-2 text-2xl font-semibold truncate max-w-[85%]">{planName}</span>
                        ...
                        <DropdownSection>
                            <DropdownItem
                                onClick={() =>
                                    dispatch(
                                        modelActions.show({
                                            name: modelNames.EDIT_STUDY_PLAN,
                                            props: { id, name: planName, description }
                                        })
                                    )
                                }
                                endContent={
                        ...
                        <DropdownSection>
                            <DropdownItem
                                onClick={() =>
                                    dispatch(
                                        modelActions.show({
                                            name: modelNames.DELETE_STUDY_PLAN,
                                            props: { id }
                                        })
                                    )
                                }
