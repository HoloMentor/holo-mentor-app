        ...
        <div className="flex flex-col gap-3">
            ...
            <Content>
                <Accordion variant="splitted" selectionMode="multiple">
                    {classTopics?.data?.map((topic: { id: string; name: string; subTopics: { id: string; name: string; isDone: boolean }[] }) => (
                        <AccordionItem
                            key={topic.id}
                        ...
                        >
                            {topic.subTopics?.length > 0 ? (
                                topic.subTopics.map((subTopic: { id: string; name: string; isDone: boolean }) => (
                                    <div
                                        key={subTopic.id}
